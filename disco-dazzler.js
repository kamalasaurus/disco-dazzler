/*!
 * native-sparkle - v0.1 - 2019-10-16
 *
 * Copyright (c) 2019 kamalasaurus
 *
 * ported from the seminal:
 *
...
DiscoDazzler = (function() {
  function startSparkle(elements, options, always) {
    var applySparkle = function(element) {
      ...
      var startSparkle = function() { sparkle.start(element) };
      var stopSparkle = function() { sparkle.stop() };
      var resizeSparkle = function() {
        sparkle.resize(element);
      ...
      if (always) {
        ...
        element.addEventListener('focus', startSparkle);
        element.addEventListener('mouseout', stopSparkle);
        element.addEventListener('blur', stopSparkle);
      }
    };

    return Array.from(elements).forEach(applySparkle);
  };
  ...
  var _ = {
    getStyle: function (domElement, attr) {
      return domElement.style.getPropertyValue(attr);
    },
    setStyle: function(domElement, attr, value) {
      return domElement.style.setProperty(attr, value);
    },
    addClass: function(domElement, _class) {
      return domElement.classList.add(_class);
    }
  };
  ...
  Sparkle.prototype = {
    "init": function(parent) {
      ...
      if (_.getStyle(parent, 'position') === 'static')
        _.setStyle(parent, 'position', 'relative');

      // create and style canvas
      this.canvas = document.createElement('canvas');
      _.addClass(this.canvas, 'sparkle-canvas');
      Object.keys(cssOpts)
        .forEach(function(key) {
          _.setStyle(this.canvas, key, cssOpts[key]);
        }.bind(this));
      _.setStyle(this.canvas, 'display', 'none');

      if (_.getStyle(parent, 'z-index') !== 'auto') {
        var zdex = parseInt(_.getStyle(parent, 'z-index'), 10);
        _.setStyle(this.canvas, 'z-index', zdex+1);
      }
      ...
      if( this.isSingleton ) {
        parent.insertAdjacentElement('afterend', this.canvas);
      } else {
      ...
      this.canvas.width = parent.offsetWidth*1.2;
      this.canvas.height = parent.offsetHeight*1.2;
      ...
      this.anim = window.requestAnimationFrame(function(time) {
        ...
        if (_this.fade) {
          ...
          if (_this.fadeCount < 0) {
            ...
            _.setStyle(_this.canvas, 'display', 'none');
          } else {
            _this.update();
          }
        } else {
    ...
    "resize": function(parent) {
      ...
      this.canvas.width = parent.offsetWidth + (this.options.overlap * 2);
      this.canvas.height = parent.offsetHeight + (this.options.overlap * 2);
      ...
      // something has moved.
      if( this.isSingleton ) {
        _.setStyle(this.canvas, 'top', parent.offsetTop - this.options.overlap);
        _.setStyle(this.canvas, 'left', parent.offsetLeft - this.options.overlap);
      }
    }, // end resize
    "start": function(parent) {
      ...
      _.setStyle(this.canvas, 'display', 'block');
